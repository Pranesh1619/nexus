      ...
      if (last10Digits.length >= 7) {
        ...
        try {
          ...
          if (recentCall && recentCall.user.phone) {
            ...
            console.log(`[INBOUND Voice Route] Routed to recently called agent: ${recentCall.user.name} (${agentPhone}) based on CallLog.`);
          }
        } catch (dbErr) {
          console.error("[INBOUND Voice Route] Error checking recent calls:", dbErr);
        }
